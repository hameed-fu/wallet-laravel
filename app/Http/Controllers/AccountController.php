...
class AccountController extends Controller
{
	...
	public function index() {
		if (Auth::user()->isrole('5')){
			$accounts = AccountModel::get();  // get all accounts for transfer purpose
	...
	public function getdata() {
		...
		->addColumn( 'action', function ( $accountsingle ) {
		        if(Auth::user()->role == "Administrator"){
		            return '
					<a href="account/transfer_history/'.$accountsingle->accountid.'" id="btnedit" customdata='.$accountsingle->accountid.' class="btn btn-sm btn-primary"><i class="ti-share"></i> Transfers</a>
					<a href="account/detail/'.$accountsingle->accountid.'" id="btnedit" customdata='.$accountsingle->accountid.' class="btn btn-sm btn-success"><i class="ti-check-box"></i>'. trans('lang.detail').'</a>
		...
		->join("account_access","account.accountid","account_access.accountid")
		->select("account.*")
		->where("account_access.userid",Auth::user()->userid)->get();
	...
	public function getdatadetailForTranfer($id){
	...
	public function transfer(Request $request){
		// current balance
		// $current_from_balance = AccountModel::where("accountid",$request->from )->value('balance');
		// $current_to_balance = AccountModel::where("accountid",$request->to )->value('balance');

		// calculate current and new balance according to from and to account
		// $from_balance =   $current_from_balance -  $request->amount;
		// $to_balance	  = $current_to_balance +  $request->amount;
		
		// DB::table("account")->where("accountid",$request->from )
		// ->update([
		// 	'balance'	=> $from_balance
		// ]);
		// DB::table("account")->where("accountid",$request->to )
		// ->update([
		// 	'balance'	=> $to_balance
		// ]);

		$from_account = AccountModel::where("accountid",$request->from )->value('name');
		$to_account = AccountModel::where("accountid",$request->to )->value('name');

		$insert = DB::table("transfers")->insert([
			'account_id'			=> $request->from,
			'transfer_account_from'	=> $from_account,
			'transfer_account_to'	=> $to_account,
			'amount'				=> $request->amount
		]);

		if ( $insert ) {
			$res['success'] = true;
			$res['message']= 'Transfer has been added';
			return response( $res );
		}
	}

	public function getAllTranferHistory($accountid){
		$data = DB::table("transfers")->where("account_id",$accountid)->get();
		return view("account.transfer_history",['data' => $data]);

	}
	
	// all tranfers

	public function getAllTranferTransaction($accountid){
		$res = DB::table("transfers")
		->where("account_id",$accountid)
		->select( DB::raw('SUM(transfers.amount) as total_transfers'))
		->get();
		$status = count($res);
		$res['success'] = true;
		$res['message']= 'Transfer has beeN listed successfully';
		return $status? response( $res ): null;

	}
	public function getAllTranferTransactionForBalance($accountid){
		$res = DB::table("transfers")
		->where("account_id",$accountid)
		->select( DB::raw('SUM(transfers.amount) as total_transfers'))
		->get();
		$acc_bal = $this->getdatadetailForTranfer($accountid);
		
		$status = count($res);
		$res['success'] = true;
		$res['message']= 'Transfer has beeN listed successfully';
		return $status? response(['balance' =>$acc_bal['balance'], 'transfers' => $res[0]->total_transfers] ): null;

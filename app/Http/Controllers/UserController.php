...
class UserController extends Controller
{
	...
	public function save(Request $request){
		...
		$account_access   = $request->input('account_access')?:null;
		...
		if($emailcheck){
			...
			if($insert){
				...
				for($x=0; $x<count($account_access); $x++){

					DB::table('account_access')->insert(
						[
						'userid'   	=> $lastid,
						'accountid' =>$account_access[$x]
						]
					);
				}
				$res['success'] = true;
	...
	public function getedit(Request $request){
		...
		$account_access   = DB::table('account_access')->where('userid',$id)->get();
		if($data){
			$res['success'] = true;
			$res['user']= $data;
			$res['role']= $datarole;
			$res['account_access']= $account_access;
			return response($res);
		}
	}
	...
	public function saveprofilebyadmin(Request $request){
		...
		if ($password !='') {
			...
			DB::table('account_access')->where('userid',$id)->delete();
			for($x=0; $x<count($account_access); $x++){

				DB::table('account_access')->insert(
					[
						'userid'   =>$id,
					'accountid' =>$account_access[$x]?:null
					]
				);
			}

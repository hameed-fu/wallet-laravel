...
$(document).ready(function() {
	...
	// subtract all transfer from actual balance

	$.ajax({
        type: "GET",
        url: "{{ url('account/getAllTranferTransaction/'.$id)}}",
        dataType: "json",
        data: "{}",
        success: function (html) {
			console.log("transd",html);
			var trans_balance = html[0].total_transfers;
			var account_balance = $(".accountbalance").html()
			account_balance = account_balance.replace(",", "");
			// console.log(account_balance-trans_balance)
			var Balance = account_balance-trans_balance;
			var num_parts = Balance.toString().split(".");
			num_parts[0] = num_parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ",");
			remainBalance = num_parts.join(".")

			$(".accountbalance").html(remainBalance)

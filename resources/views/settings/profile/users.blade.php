...
$(document).ready(function() {
	...
	$.ajax({
        ...
        success: function (html) {
			...
			$.each(objs, function(index, object) {
				...
				$.each(accounts, function(index, object) {					
					accnts += '<div class="checkbox">'+
						  '<label><input  type="checkbox" id="chk'+object.accountid+'" class="access" name="account_access[]" id="access" value="' + object.accountid + '">'+object.name+'</label>'+
							'</div>';
				});
				$('.accounts').html(accnts);

				$.each(accounts, function(index, object) {
					accnts2 += '<div class="checkbox">'+
						  '<label><input type="checkbox" id="achk'+object.accountid+'" class="access" name="access[]" id="access" value="' + object.accountid + '">'+object.name+'</label>'+
							'</div>';
				});
				$('.accounts2').html(accnts2);
	...
	$("#save").click(function(e){
		...
		var dataroles   = []; 	
		var account_access   = ["0"]; 	
		$("input:checked").each(function() {
		   dataroles.push($(this).val());
		});

		$('.accounts :checkbox:checked').each(function(i){
			account_access[i] = $(this).val();
		});

		  console.log(account_access)
		...
		$.ajax({
            ...
            data: {name:name,password:password,email:email,phone:phone,role:role,status:status,dataroles,account_access:account_access},
            dataType: "JSON",
	...
	$("#saveedit").click(function(e){
		...
		$('.accounts2 :checkbox:checked').each(function(i){
			account_access[i] = $(this).val();
		...
		$.ajax({
            ...
            data: {id:id,name:name,email:email,phone:phone,password:password,status:status,role:role,dataroles,account_access:account_access},
            dataType: "JSON",
            success: function(data) {
				//$("#message").html(data);
		...
		$('#edit').on('show.bs.modal', function(e) {
			...
			$.ajax({
				...
				success: function(data) {
					console.log(data)
					var objs =data.role;
					var account_access =data.account_access;
					$("#idedit").val(id);
					...
					$.each(account_access, function(index, object) {						
							$('#achk'+object.accountid).each(function(){
								if(this.value = object.accountid) {
									$('#achk'+object.accountid).prop( "checked", true );
								}
							});
					});
				}
